...
static void CL_UpdateMumble(void) {
	...
	if (cl_useMumble->integer > 1) {
		fprintf(stderr, "%f %f %f, %f %f %f, %f %f %f\n", pos[0], pos[1], pos[2], forward[0], forward[1], forward[2],
				up[0], up[1], up[2]);
	}
...
void CL_Voip_f(void) {
	...
	if (clc.state != CA_ACTIVE)
		...
		reason = "Server doesn't support VoIP";
			 ...
			 (Cvar_VariableValue("g_gametype") == GT_SINGLE_PLAYER || Cvar_VariableValue("ui_singlePlayerActive")))
...
void CL_AddReliableCommand(const char *cmd, qboolean isDisconnectCmd) {
	...
	Q_strncpyz(clc.reliableCommands[++clc.reliableSequence & (MAX_RELIABLE_COMMANDS - 1)], cmd,
			   sizeof(*clc.reliableCommands));
}
...
static void CL_RequestMotd(void) {
	...
	Com_Printf("%s resolved to %i.%i.%i.%i:%i\n", UPDATE_SERVER_NAME, cls.updateServer.ip[0], cls.updateServer.ip[1],
	...
	Com_sprintf(cls.updateChallenge, sizeof(cls.updateChallenge), "%i",
				(int)((((unsigned int)rand() << 16) ^ (unsigned int)rand()) ^ Com_Milliseconds()));
...
void CL_CheckForResend(void) {
	...
	case CA_CONNECTING:
		...
		// connection. Add the gamename so the server knows we're running the correct game or can reject the client with
...
void CL_Frame(int msec) {
	...
	if (cl_autoRecordDemo->integer) {
		if (clc.state == CA_ACTIVE && !clc.demorecording && !clc.demoplaying) {
			...
			Com_RealTime(&now);
			nowString = va("%04d%02d%02d%02d%02d%02d", 1900 + now.tm_year, 1 + now.tm_mon, now.tm_mday, now.tm_hour,
